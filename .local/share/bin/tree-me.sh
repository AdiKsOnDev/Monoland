...
# Worktree organization: ../<branch> (relative to git repo root)
# Get git repo root and use its parent directory
GIT_ROOT="$(git rev-parse --show-toplevel 2>/dev/null || pwd)"
WORKTREE_ROOT="${WORKTREE_ROOT:-$(dirname "$GIT_ROOT")}"
...
Worktrees are organized at: ../<branch> (relative to git repo root)
Set WORKTREE_ROOT to customize the location (default: parent of git root)
EOF
}
